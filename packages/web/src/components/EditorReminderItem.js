...
import {
  ...
  updateEditorReminderSound, updateEditorReminderRepetitions,
  updateEditorReminderInterval,
} from '../actions';
...
const EditorReminderItem = (props) => {
  ...
  const onMessageInputChange = (e) => {
    dispatch(updateEditorReminderMessage(e.target.value, reminder.key));
  };

  const onMessageDisplayDurationInputChange = (e) => {
    dispatch(updateEditorReminderMessageDisplayDuration(e.target.value, reminder.key));
  };

  const onSoundInputChange = (e) => {
    dispatch(updateEditorReminderSound(e.target.value, reminder.key));
  };
      ...
      <React.Fragment>
        <motion.tr layoutId={`${reminder.key}-2`} className="h-14">
          ...
          <td className="px-6 py-2.5">
            ...
            <div className="mt-1">
              <input onChange={onMessageInputChange} type="text" name={`reminder-message-${reminder.key}`} id={`reminder-message-${reminder.key}`} className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm text-gray-900 border-gray-300 rounded-md py-1" />
            </div>
          </td>
          <td colSpan={2} className="px-6 py-2.5">
            <label htmlFor={`reminder-display-duration-${reminder.key}`} className="block text-sm font-medium text-gray-700">Reminder display duration</label>
            <div className="mt-1">
              <input onChange={onMessageDisplayDurationInputChange} type="text" name={`reminder-display-duration-${reminder.key}`} id={`reminder-display-duration-${reminder.key}`} className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm text-gray-900 border-gray-300 rounded-md py-1" />
            </div>
          </td>
        </motion.tr>
        <motion.tr layoutId={`${reminder.key}-3`} className="border-b border-gray-200">
          <td></td>
          <td colSpan={3} className="px-6 pt-2.5 pb-4">
            <label htmlFor={`reminder-sound-${reminder.key}`} className="block text-sm font-medium text-gray-700">Reminder sound</label>
            <div className="mt-1 flex">
              <input onChange={onSoundInputChange} type="text" name={`reminder-sound-${reminder.key}`} id={`reminder-sound-${reminder.key}`} className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm text-gray-900 border-gray-300 rounded-md py-1" />
    ...
    <React.Fragment>
      <motion.tr layoutId={`${reminder.key}-1`} className={rowClassNames}>
        ...
        <td className={`px-6 whitespace-nowrap text-sm font-medium text-gray-500 truncate ${colClassNames}`}>
          <input onChange={onRepetitionsInputChange} type="text" name={`reminder-repetitions-${reminder.key}`} id={`reminder-repetitions-${reminder.key}`} className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm text-gray-900 border-gray-300 rounded-md py-1" value={reminder.repetitions} />
        </td>
        <td className={`px-6 whitespace-nowrap text-sm font-medium text-gray-500 truncate ${colClassNames}`}>
          <input onChange={onIntervalInputChange} type="text" name={`reminder-interval-${reminder.key}`} id={`reminder-interval-${reminder.key}`} className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm text-gray-900 border-gray-300 rounded-md py-1" value={reminder.interval} />
        </td>
      ...
      {reminder.isMoreOptionsShown && moreOptions}
    </React.Fragment>
  );
};
