...
import {
  ...
  updateEditorIsMoreSettingsShown, updateEditorSelectingReminderKey,
  saveTimer,
} from '../actions';
import {
...
const EditorForm = () => {
  ...
  const nextTimerName = useSelector(state => getNextTimerName(state));
  ...
  const reminderMessageDisplayDurationErrMsg = useSelector(
    state => state.editor.reminderMessageDisplayDurationErrMsg
  );
  const nameInput = useRef(null);
  const reminderMessageInput = useRef(null);
  const reminderSoundBtn = useRef(null);
  const nextTimerBtn = useRef(null);
  const nextTimerStartsByBtn = useRef(null);
  const didClick = useRef(null);
  ...
  const onReminderSoundBtnClick = () => {
    dispatch(updateEditorSelectingReminderKey(null));
    updatePopupUrlHash(
  ...
  const onPlayBtnClick = () => {
    const sound = SOUNDS.find(sound => sound.name === reminderSound);
    if (sound) {
  ...
  useEffect(() => {
    if (isShown) setTimeout(() => nameInput.current.focus(), 100);
    else {
  ...
  useEffect(() => {
    if (isMoreSettingsShown) setTimeout(() => reminderMessageInput.current.focus(), 100);
  }, [isMoreSettingsShown]);

  useEffect(() => {
    return () => {
      if (didClick.current) clearTimeout(didClick.current);
    };
  }, []);

  const reminderMessageDisplayDurationInputClassNames = reminderMessageDisplayDurationErrMsg ? 'border-red-300 text-red-900 placeholder-red-300 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 text-gray-900 focus:ring-indigo-500 focus:border-indigo-500';
    ...
    moreSettings = (
      <div className="mt-8 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
        <div className="sm:col-span-3">
          ...
          <div className="mt-1">
            <input ref={reminderMessageInput} onChange={onReminderMessageInputChange} type="text" name="reminder-message" id="reminder-message" className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm text-gray-900 border-gray-300 rounded-md" value={reminderMessage} />
          </div>
        </div>
        <div className="sm:col-span-3">
          <label htmlFor="reminder-message-display-duration" className="block text-sm font-medium text-gray-700">Reminder display duration (in seconds)</label>
          <div className="mt-1 relative rounded-md shadow-sm">
            <input onChange={onReminderDisplayDurationInputChange} type="text" name="reminder-message-display-duration" id="reminder-message-display-duration" className={`block w-full pr-10 focus:outline-none sm:text-sm rounded-md ${reminderMessageDisplayDurationInputClassNames}`} value={reminderMessageDisplayDuration} />
            {reminderMessageDisplayDurationErrMsg && <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
              <svg className="h-5 w-5 text-red-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
              </svg>
            </div>}
          </div>
          {reminderMessageDisplayDurationErrMsg && <p className="mt-2 text-sm text-red-600" id="reminder-message-display-duration-error">{reminderMessageDisplayDurationErrMsg}</p>}
        </div>
  ...
  const durationInputClassNames = durationErrMsg ? 'border-red-300 text-red-900 placeholder-red-300 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 text-gray-900 focus:ring-indigo-500 focus:border-indigo-500';
    ...
    <div className="p-8">
      ...
      <div className="mt-8 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
        ...
        <div className="sm:col-span-3">
          ...
          <div className="mt-1 relative rounded-md shadow-sm">
            <input onChange={onDurationInputChange} type="text" name="duration" id="duration" className={`block w-full pr-10 focus:outline-none sm:text-sm rounded-md ${durationInputClassNames}`} placeholder="00:00" value={duration} />
          ...
          {durationErrMsg && <p className="mt-2 text-sm text-red-600" id="duration-error">{durationErrMsg}</p>}
        </div>
      </div>
      ...
      {moreSettings}
      <button onClick={onSaveBtnClick} className="w-full mt-8 bg-indigo-600 border border-transparent rounded-md shadow-sm py-2 px-4 text-sm font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" type="button">Save</button>
    </div>
  );
};
